...
このブログは日本語と英語の両方に対応しています。

### 機能

- URLパスによる言語切り替え（例: `/ja/about` - 日本語のAboutページ）
- ヘッダーに言語切り替えリンク
- 日付表示の日本語対応
- 各コンテンツの翻訳
...
新しい翻訳を追加するには、以下のフォルダに翻訳ファイルを作成します：

```
public/locales/[言語コード]/[名前空間].json
```

例えば、ブログ関連の日本語翻訳は以下のように配置されています：

```
public/locales/ja/blog.json
```
